...
from problem_generation import make_nonlinear_nonstiff_schrodinger_ode
...
for i, size in enumerate(sizes):
    ...
    ode, X0 = make_nonlinear_nonstiff_schrodinger_ode(size)
    ...
    for rank in ranks[i]:
        ...
        for j, method in enumerate(dlra_solvers):
            ...
            print(f'Computation time for {dlra_names[j]}: {np.sum(Ys.computation_time)}')
        ...
        for j, method in enumerate(dlra_deim_solvers):
            ...
            print(f'Computation time for {dlra_deim_names[j]}: {np.sum(Ys.computation_time)}')
