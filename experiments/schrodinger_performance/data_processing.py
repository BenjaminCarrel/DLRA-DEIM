...
for i, size in enumerate(sizes):
    ...
    for j, rank in enumerate(ranks[i]):
        ...
        for k, method in enumerate(dlra_names):
            ...
            final_errors[k+1, i*len(ranks)+j] = np.linalg.norm(dlra_sol.Xs[-1] - Xs_ref[-1], 'fro') / np.linalg.norm(Xs_ref[-1], 'fro')
            time_of_computation[k+1, i*len(ranks)+j] = np.sum(dlra_sol.computation_time)
