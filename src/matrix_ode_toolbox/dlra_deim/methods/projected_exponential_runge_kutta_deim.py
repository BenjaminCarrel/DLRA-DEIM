...
Projected Exponential Runge-Kutta methods accelerated by the DEIM for solving stiff Sylvester-like matrix differential equations.
See Carrel & Vandereycken 2024.
"""
...
#%% Class DeimProjected Exponential Runge-Kutta
class ProjectedExponentialRungeKuttaDeim(DlraDeimSolver):
    """
    DEIM Projected exponential Runge-Kutta methods.
    ...
    #%% Class attributes
    # Name of the method
    name = 'Projected exponential Runge-Kutta DEIM'
    ...
    def __init__(self,
        ...
        if 'scipy_method' in extra_args:
            ...
            self.scipy_method = 'LSODA' # default
        # If the rank decreases, we need to store the original rank
        self.rank = None
        self.extra_args = extra_args

        # Timer for the different steps
        self.timer = {'Krylov spaces': 0,
                      'Application of exponential': 0,
                      'Sylvester (order one)': 0,
                      'Sylvester (order two)': 0,
                      'Closed form solver (order one)': 0,
                      'Closed form solver (order two)': 0,
                      'Scipy solver (order one)': 0,
                      'Scipy solver (order two)': 0}
        
        # Add this to the extra_data
        self.extra_data['timer'] = self.timer
    ...
    def info(self) -> str:
        ...
        info = f'Projected exponential Runge-Kutta accelerated by the DEIM (PERK-DEIM) \n'
    ...
    def DEIM_non_linear_shortcut(self, t0: float, Y0: SVD, U_indexes: list, V_indexes: list) -> tuple:
        "Shortcut for non linear evaluation with DEIM"
        try: # evaluate only on the DEIM indices
            GY0_u = self.matrix_ode.non_stiff_field(t0, Y0, rows=U_indexes)
            GY0_v = self.matrix_ode.non_stiff_field(t0, Y0, cols=V_indexes)
            GY0_uv = self.matrix_ode.non_stiff_field(t0, Y0, rows=U_indexes, cols=V_indexes)
        except: # evaluate on the whole matrix
            print('Warning: pointwise evaluation of the non-stiff field is not implemented. The whole matrix is evaluated.')
            GY0 = self.matrix_ode.non_stiff_field(t0, Y0)
            GY0_u = GY0[U_indexes, :]
            GY0_v = GY0[:, V_indexes]
            GY0_uv = GY0_u[:, V_indexes]
        return GY0_u, GY0_v, GY0_uv

    def stepper(self, t_subspan: tuple, Y0: SVD) -> SVD:
    ...
    def retraction(self, X: QuasiSVD) -> SVD:
    ...
    def compute_Krylov_basis(self, matrices: list):
        ...
        self.timer['Krylov spaces'] += time.time() - tic
        return Vk, Wk
    ...
    def exp_sylvester(self, h: float, A: Matrix, B: Matrix, X: Matrix) -> Matrix:
        ...
        self.timer['Application of exponential'] += time.time() - tic
        return Z
    ...
    def solve_sylvester_one_ode(self, h: float, A: Matrix, B: Matrix, X0: SVD, X1: SVD) -> QuasiSVD:
        ...
        if self.use_closed_form:
            ...
            S1 = Z - C
            self.timer['Closed form solver (order one)'] += time.time() - tic
        ...
        ## Solve reduced ODE (default solver is scipy's LSODA for automatic stiffness detection)
            ...
            self.timer['Scipy solver (order one)'] += time.time() - tic
            output = QuasiSVD(Qk, S1, Wk)
        self.timer['Sylvester (order one)'] += time.time() - tic
        return output
    ...
    def solve_sylvester_two_ode(self, h: float, A: Matrix, B: Matrix, X0: SVD, X1: SVD, X2: SVD) -> QuasiSVD:
        ...
        if self.use_closed_form:
            ...
            X01 = np.linalg.multi_dot([NA, NA.T.conj(), X0_reduced, NB, NB.T.conj()])
            X02 = np.linalg.multi_dot([NA, NA.T.conj(), X0_reduced, OB, OB.T.conj()])
            X03 = np.linalg.multi_dot([OA, OA.T.conj(), X0_reduced, NB, NB.T.conj()])
            X04 = np.linalg.multi_dot([OA, OA.T.conj(), X0_reduced, OB, OB.T.conj()])
            X11 = np.linalg.multi_dot([NA, NA.T.conj(), X1_reduced, NB, NB.T.conj()])
            X12 = np.linalg.multi_dot([NA, NA.T.conj(), X1_reduced, OB, OB.T.conj()])
            X13 = np.linalg.multi_dot([OA, OA.T.conj(), X1_reduced, NB, NB.T.conj()])
            X14 = np.linalg.multi_dot([OA, OA.T.conj(), X1_reduced, OB, OB.T.conj()])
            X21 = np.linalg.multi_dot([NA, NA.T.conj(), X2_reduced, NB, NB.T.conj()])
            X22 = np.linalg.multi_dot([NA, NA.T.conj(), X2_reduced, OB, OB.T.conj()])
            X23 = np.linalg.multi_dot([OA, OA.T.conj(), X2_reduced, NB, NB.T.conj()])
            X24 = np.linalg.multi_dot([OA, OA.T.conj(), X2_reduced, OB, OB.T.conj()])
            ...
            # Assemble
            S = S1 + S2 + S3 + S4
            self.timer['Closed form solver (order two)'] += time.time() - tic
        ...
        ## Solve reduced ODE (with scipy LSODA for automatic stiffness detection)
            ...
            self.timer['Scipy solver (order two)'] += time.time() - tic
            output = QuasiSVD(Qk, S2, Wk)
        self.timer['Sylvester (order two)'] += time.time() - tic
        return output
    ...
    def projected_exponential_Euler(self, t_subspan: tuple, Y0: SVD) -> SVD:
        ...
        # COMPUTE THE OBLIQUE PROJECTION: P^{DEIM}(Y0)[G(Y0)]
        U_indexes, M_u = self.indexes_U, self.M_U
        V_indexes, M_v = self.indexes_V, self.M_V
        GY0_u, GY0_v, GY0_uv = self.DEIM_non_linear_shortcut(t0, Y0, U_indexes, V_indexes)
        PGY0 = Y0.project_onto_DEIM_tangent_space(GY0_u, GY0_v, GY0_uv, M_u, M_v, truncate=self._allow_automatic_truncation)

        # COMPUTE THE INNER TERM: exp(h L) (Y0) + h phi(h L) (P(Y0)[G(Y0)])
    ...
    def projected_exponential_Runge_non_strict(self, t_subspan: tuple, Y0: SVD) -> SVD:
        ...
        # Update the DEIM indexes and matrices if needed
        if self._perform_deim_at_each_stage:
            U_indexes, M_u = self.DEIM_shortcut(Y_half.U)
            V_indexes, M_v = self.DEIM_shortcut(Y_half.V)
        else:
            U_indexes, M_u = self.indexes_U, self.M_U
            V_indexes, M_v = self.indexes_V, self.M_V
        # STEP 2: COMPUTE THE OBLIQUE PROJECTION: P^{DEIM}(Y_half)[G(Y_half)]
        GY_half_u, GY_half_v, GY_half_uv = self.DEIM_non_linear_shortcut(t0 + h_half, Y_half, U_indexes, V_indexes)
        PGY_half = Y_half.project_onto_DEIM_tangent_space(GY_half_u, GY_half_v, GY_half_uv, M_u, M_v, truncate=self._allow_automatic_truncation)
    ...
    def projected_exponential_Runge_strict(self, t_subspan: tuple, Y0: SVD) -> SVD:
        ...
        # Update the DEIM indexes and matrices if needed
        if self._perform_deim_at_each_stage:
            U_indexes, M_u = self.DEIM_shortcut(RK1.U)
            V_indexes, M_v = self.DEIM_shortcut(RK1.V)
        else:
            U_indexes, M_u = self.indexes_U, self.M_U
            V_indexes, M_v = self.indexes_V, self.M_V

        # STEP 2
        ## Compute the oblique projection: P^{DEIM}(RK1)[G(RK1)]
        GRK1_u, GRK1_v, GRK1_uv = self.DEIM_non_linear_shortcut(t0 + c2 * h, RK1, U_indexes, V_indexes)
        PGRK1 = RK1.project_onto_DEIM_tangent_space(GRK1_u, GRK1_v, GRK1_uv, M_u, M_v, truncate=self._allow_automatic_truncation)
