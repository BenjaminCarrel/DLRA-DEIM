...
class ProjectedExponentialRungeKutta(DlraSolver):
    ...
    def projected_exponential_Euler(self, t_subspan: tuple, Y0: SVD) -> SVD:
        ...
        PGY0 = Y0.project_onto_tangent_space(GY0, truncate=self.automatic_truncation)
    ...
    def projected_exponential_Runge_non_strict(self, t_subspan: tuple, Y0: SVD) -> SVD:
        ...
        PGY_half = Y_half.project_onto_tangent_space(GY_half, truncate=self.automatic_truncation)
    ...
    def projected_exponential_Runge_strict(self, t_subspan: tuple, Y0: SVD) -> SVD:
        ...
        PGRK1 = RK1.project_onto_tangent_space(GRK1, truncate=self.automatic_truncation)
    ...
    def projected_exponential_Runge_robust(self, t_subspan: tuple, Y0: SVD) -> SVD:
        """
        Projected exponential Runge method. Version 2, which verifies the stiff order conditions.
        Solves an equation of the form X' = AX + XB + G(X) = L(X) + G(X)
        The scheme is:
        ...
        # COMPUTE THE PROJECTION: P(Y0)[G(Y0)]
        GY0 = self.matrix_ode.non_stiff_field(0, Y0)
        PGY0 = Y0.project_onto_tangent_space(GY0, truncate=self.automatic_truncation)
        # Combined phi_k with full matrices
        K1: SVD = sylvester_combined_phi_k(self.matrix_ode.A, self.matrix_ode.B, h, k=1, X=[Y0, PGY0])
        RK1 = K1.truncate(Y0.rank) # RETRACT

        # COMPUTE THE PROJECTION: P(RK1)[G(RK1)]
        GRK1 = self.matrix_ode.non_stiff_field(h, RK1)
        PGRK1 = RK1.project_onto_tangent_space(GRK1, truncate=self.automatic_truncation)
        Y1: SVD = sylvester_combined_phi_k(self.matrix_ode.A, self.matrix_ode.B, h, k=2, X=[Y0, PGY0, 1/h * (PGRK1 - PGY0)])
